...
public class ProvidedIdFieldMapper extends IdFieldMapper {
    ...
    public void mapMetadataColumns(BatchDocumentParserContext[] contexts, ColumnBatchBuilder out) {
        ...
        // Mirror preParse: in columnar mode _id is indexed + BINARY doc values (read by _id field data,
        // e.g. when fetching from an index-sorted segment); otherwise it is indexed + stored.
        final IndexableFieldType idFieldType = mode == Mode.COLUMNAR ? ColumnarIdField.TYPE : ID_COLUMN_FIELD_TYPE;
        out.addColumn(EicfLuceneColumns.arrayBinaryColumn(ids, NAME, idFieldType));
    }
