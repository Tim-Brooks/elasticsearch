...
/**
 * An EICF column whose values are all raw binary bytes, stored as a contiguous {@code data} payload
 * with a {@code (docCount + 1)}-entry offset vector delimiting each document's bytes
 ...
 */
public final class EicfBinaryColumn extends EicfColumn {

    private final byte[] data;
    private final int base;
    private final int[] offsets;

    EicfBinaryColumn(int columnIndex, int docCount, FixedBitSet absent, byte[] data, int base, int[] offsets) {
