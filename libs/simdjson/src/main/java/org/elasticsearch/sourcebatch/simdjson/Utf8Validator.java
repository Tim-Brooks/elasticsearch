...
import static org.elasticsearch.sourcebatch.simdjson.VectorUtils.BYTE_SPECIES;
import static org.elasticsearch.sourcebatch.simdjson.VectorUtils.INT_SPECIES;
...
class Utf8Validator {
    ...
    // UNSIGNED_GE(a, INCOMPLETE_CHECK) == signed GE with both sides XOR'd by 0x80. Precompute to avoid JDK version dependency.
    private static final ByteVector INCOMPLETE_CHECK_XOR_80 = INCOMPLETE_CHECK.lanewise(XOR, (byte) 0x80);
    private static final byte LOW_NIBBLE_MASK = 0b0000_1111;
    private static final byte ALL_ASCII_MASK = (byte) 0b1000_0000;
    // iota builds a "shift forward one lane" shuffle — species-generic, works at 128/256/512 bits alike.
    ...
    static void validate(byte[] buffer, int length) {
        ...
        for (; offset < loopBound; offset += STEP_SIZE) {
            ...
            if (chunk.and(ALL_ASCII_MASK).compare(EQ, 0).allTrue()) {
                ...
                previousIncomplete = chunk.lanewise(XOR, (byte) 0x80).compare(GE, INCOMPLETE_CHECK_XOR_80).toLong();
                ...
                ByteVector previousOneByte = chunkAsInts.lanewise(LSHL, Byte.SIZE)
                    .or(chunkWithPreviousFourBytes.lanewise(LSHR, THREE_BYTES_SIZE))
                    .reinterpretAsBytes();
                ByteVector byte2HighNibbles = chunkAsInts.lanewise(LSHR, 4).reinterpretAsBytes().and(LOW_NIBBLE_MASK);
                ...
                ByteVector previousTwoBytes = chunkAsInts.lanewise(LSHL, TWO_BYTES_SIZE)
                ...
                VectorMask<Byte> is3ByteLead = previousTwoBytes.and((byte) 0xE0).compare(EQ, (byte) 0xE0);
                ByteVector previousThreeBytes = chunkAsInts.lanewise(LSHL, THREE_BYTES_SIZE)
                ...
                VectorMask<Byte> is4ByteLead = previousThreeBytes.and((byte) 0xF0).compare(EQ, (byte) 0xF0);
        ...
        if (!chunk.and(ALL_ASCII_MASK).compare(EQ, 0).allTrue()) {
            ...
            ByteVector previousOneByte = chunkAsInts.lanewise(LSHL, Byte.SIZE)
                ...
                .reinterpretAsBytes();
            ByteVector byte2HighNibbles = chunkAsInts.lanewise(LSHR, 4).reinterpretAsBytes().and(LOW_NIBBLE_MASK);
            ByteVector byte1HighNibbles = previousOneByte.reinterpretAsInts().lanewise(LSHR, 4).reinterpretAsBytes().and(LOW_NIBBLE_MASK);
            ByteVector byte1LowNibbles = previousOneByte.and(LOW_NIBBLE_MASK);
            ByteVector byte1HighState = byte1HighNibbles.selectFrom(BYTE_1_HIGH_LOOKUP);
            ByteVector byte1LowState = byte1LowNibbles.selectFrom(BYTE_1_LOW_LOOKUP);
            ByteVector byte2HighState = byte2HighNibbles.selectFrom(BYTE_2_HIGH_LOOKUP);
